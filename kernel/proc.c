...
void q_push(struct Queue *qu, struct proc *p) {
    qu->q[qu->tail] = p;
    qu->tail = (qu->tail + 1) % qu->max;
    qu->size++;
}

void q_pop(struct Queue *qu) {
...
PUBLIC void schedule() {
    ...
    while (queue_1.size > 0) {
        p = queue_1.q[queue_1.front];

        if (p->p_flags == FREE_SLOT)
            q_pop(&queue_1);
        else {
            if (p->p_flags == 0 && p->ticks) {
                proc_timeslice = queue_1.timeslice;
                break;
            } else {
                q_pop(&queue_1);
                q_push(&queue_3, p);
    ...
    while (queue_1.size == 0 && queue_2.size > 0) {
        p = queue_2.q[queue_2.front];

        if (p->p_flags == FREE_SLOT)
            q_pop(&queue_2);
        else {
            if (p->p_flags == 0 && p->ticks) {
                proc_timeslice = queue_2.timeslice;
                break;
            } else {
                q_pop(&queue_2);
                q_push(&queue_3, p);
    ...
    while (queue_1.size == 0 && queue_2.size == 0 && queue_3.size > 0) {
        p = queue_3.q[queue_3.front];

        if (p->p_flags == FREE_SLOT)
            q_pop(&queue_3);
        else {
            if (p->p_flags == 0 && p->ticks) {
                proc_timeslice = queue_3.timeslice;
                break;
            } else {
                q_pop(&queue_3);
                q_push(&queue_3, p);
